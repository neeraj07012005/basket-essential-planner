...
export default function WithoutBudget(){
...
const filtered=itemsInCategory.filter(i=>
...
let note="Shopping List (Without Budget)\n\n";
...
<Text style={styles.title}>Without Budget</Text>
...
<View style={{flex:1}}>
<Text>{item.category} - {item.itemName}</Text>
<Text>{item.quantity} {item.unit} × ₹{item.price}</Text>
